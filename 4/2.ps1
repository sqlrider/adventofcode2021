...
# Create array of cards, then need to initialise each card
...
foreach($bingonumber in $bingonumbers)
{
    for($a = 0; $a -lt 100; $a++)
    {
        ...
        if($card[$a].won -eq $false)     # Don't check a card that's already won - change for Part 2, as we keep on checking instead of quitting after finding first winner
        {
